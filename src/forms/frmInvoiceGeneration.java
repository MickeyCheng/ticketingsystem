...
public class frmInvoiceGeneration extends javax.swing.JFrame {
ResultSet rs;
Connection conn;
...
String getItemCode;
    ...
    private void fillTableService(){
        try{
            ...
            tblService.getColumnModel().getColumn(1).setHeaderValue("ITEM");
...
//        conn = DriverManager.getConnection("jdbc:mysql://192.168.1.13/dbticketing","iSoft","iSoft123");
    ...
    private void getNextQuoteID(){
        try{
            pstmt = conn.prepareStatement("SELECT * from tblInvoiceGenerator order by ig_id DESC LIMIT 1");
            rs = pstmt.executeQuery();
            if (rs.next()){
                getMaxQuoteID = rs.getInt("ig_id");
                getMaxQuoteID++;
            }else{
                getMaxQuoteID=1;
            }
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        getNextQuoteID();
        String insertQuote = "INSERT INTO tblInvoiceGenerator (ig_id,ig_clientName,ig_clientCompany,ig_clientAddress,"
                + "ig_clientContact,ig_clientEmail,ig_itemCode,ig_itemName,ig_quantity,ig_price,"
                + "ig_totalPrice,ig_totalQuantity,ig_totalInvoiceAmount,ig_quoteDate)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try{
            for(int i=0; i<tblQuote.getRowCount();i++){
            ...
            JOptionPane.showMessageDialog(this, "INVOICE CREATED AND SAVED");
            printInvoice();
        }catch(SQLException e){
            JOptionPane.showMessageDialog(this, e.getMessage());
        }            
    ...
    private void printInvoice(){
            ...
            try{
                ...
                Class.forName("com.mysql.jdbc.Driver");
                conn = DriverManager.getConnection("jdbc:mysql://192.168.100.2/dbticketing","iSoft","iSoft123");
                JasperDesign jd = JRXmlLoader.load(new File("src\\Reports\\repInvoice.jrxml"));
    ...
    private void listenSearch(){
        try{
            String searchQuery = "Select * from tblProduct where sr_name like ?";
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(frmInvoiceGeneration.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new frmInvoiceGeneration().setVisible(true);
            }
        });
    }
